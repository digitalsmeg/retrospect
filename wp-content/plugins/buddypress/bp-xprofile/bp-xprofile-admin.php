...
class BP_XProfile_User_Admin {
	...
	public function user_admin_profile_metaboxes( $user = null, $args = array() ) {
		...
		<label >Terms</label>
		<input type='checkbox' 
        onclick="setExtTermsAgreements(<? echo $r[user_id]; ?>,'terms')" 
        value='<? echo $r[user_id]; ?>' 
		<? if(!empty(get_user_meta($r[user_id], "retro_terms", true))){ ?>checked<? } ?> /> 
        <span id="terms_date"><? echo get_user_meta($r[user_id], "retro_terms_date", true); ?></span>
		</div>  
        
          <div class="bp-profile-field">
		<label >Privacy Policy</label>
		<input type='checkbox' 
        onclick="setExtTermsAgreements(<? echo $r[user_id]; ?>,'privacy')" 
        value='<? echo $r[user_id]; ?>' 
		<? if(!empty(get_user_meta($r[user_id], "retro_privacy", true))){ ?>checked<? } ?> /> 
        <span id="privacy_date"><? echo get_user_meta($r[user_id], "retro_privacy_date", true); ?></span>
		</div>  
        
          <div class="bp-profile-field">
		<label >Beta Agreement</label>
		<input type='checkbox' 
        onclick="setExtTermsAgreements(<? echo $r[user_id]; ?>,'beta')" 
        value='<? echo $r[user_id]; ?>' 
		<? if(!empty(get_user_meta($r[user_id], "retro_beta", true))){ ?>checked<? } ?> /> 
        <span id="beta_date"><? echo get_user_meta($r[user_id], "retro_beta_date", true); ?></span>
		</div>  
...
function setExtTermsAgreements(id,type){
                ...
                jQuery.post(ajaxurl, {action: 'setExtTermsAgreements', id: id,type: type}, function(response) {
                	jQuery("#"+type+"_date").html(response);
                });	
            
        }
function setExtMeta(id,key){
	 var v = jQuery("#"+key).find("option:selected").val();
	 if(key == "gender"){
