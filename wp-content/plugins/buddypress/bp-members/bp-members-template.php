...
foreach ( (array) $bp->bp_nav as $user_nav_item ) {
	switch($user_nav_item[slug]){
		case "activity":
			$groupedNavs[1][] = $user_nav_item;
			break;	
		case "profile":
			$groupedNavs[0][] = $user_nav_item;
			break;
		case "notifications":
			$groupedNavs[1][] = $user_nav_item;
			break;
...
ksort($groupedNavs);
	...
	foreach ($groupedNavs as $key=>$array ) {
		?><div class="item-list-tabs no-ajax" id="object-nav" role="navigation" style="margin-bottom:5px;"><ul><?
		foreach ($array as $user_nav_item ) {
		...
		echo apply_filters_ref_array( 'bp_get_displayed_user_nav_' . $user_nav_item['css_id'], array( '<li class="'.$user_nav_item['css_id'].'" id="' . $user_nav_item['css_id'] . '-personal-li" ' . $selected . '><a id="user-' . $user_nav_item['css_id'] . '" href="' . $link . '">' . $user_nav_item['name'] . '</a></li>', &$user_nav_item ) );
