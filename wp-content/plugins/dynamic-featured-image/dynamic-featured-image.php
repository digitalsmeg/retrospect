...
class Dynamic_Featured_Image
{
		...
		if ( !is_null($i) ) {
			foreach ($postTypes as $type) {
				...
				add_meta_box(
					...
					__('Secondary Image for Home', $this->_textDomain),
			...
			foreach ($postTypes as $type) {
				...
				add_meta_box(
					'dfiFeaturedMetaBox',
					__( 'Secondary Image for Home', $this->_textDomain ) . " " . __(2, $this->_textDomain),
					array( $this, 'featured_meta_box' ),
					$type,
					'side',
					'low',
	...
	private function _get_featured_box($featuredImgTrimmed, $featuredImg, $featuredId, $thumbnail, $postId)
	{
		...
		return "<a href='javascript:void(0)' class='dfiFeaturedImage {$hasFeaturedImage}' title='". __('Set Featured Image', $this->_textDomain) . "' data-post-id='" . $postId . "'><span class='dashicons dashicons-camera'></span></a><br/>
			...
			<div class='dfiLinks'>
				<a style='display:none;' href='javascript:void(0)' data-id='{$featuredId}' data-id-local='" . $this->_get_number_translation( ($featuredId + 1) ) . "' class='dfiAddNew dashicons dashicons-plus' title='" . __('Add New', $this->_textDomain) ."'></a>
